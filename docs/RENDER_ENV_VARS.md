...
## 3. تقييد الجهاز / الـ IP (الطلاب)

الطلاب يسجّلون من جهاز واحد فقط؛ الوصول من أجهزة أخرى يتطلب صلاحية **السماح بالوصول من أجهزة متعددة** من المدير.

- يُحفظ **IP التسجيل** تلقائياً عند إنشاء الحساب.
- لا تحتاج متغيرات بيئة إضافية. Render يمرّر عنوان الـ IP الحقيقي عبر `X-Forwarded-For` والـ Backend يقرؤه.

---

## 4. باقي المتغيرات (كما هي)
